 ...
 * These normally contain dashes and look like: 3-598-21508-8
    ...
    or an X only). In the case the check character is an X, this represents the value 
    '10'. These may be communicated with or without hyphens, and can be checked for 
    ...
    Bonus tasks
        ...
        Generate a valid ISBN-13 from the input ISBN-10 (and maybe verify it again with a
         derived verifier).
...
class IsbnVerifier {
    ...
    boolean isValid(String stringToVerify) {
        if (stringToVerify == null || stringToVerify.isEmpty() || digitCounter(stringToVerify) != 10) {
            return false;
        }
        ...
        for (int i = 0; i < stringToVerify.length(); i++) {
            ...
            if (Character.isDigit(c)) {
                isbnArray[i - hyphenCounter] = Character.getNumericValue(c);

            } else if (c == '-') {
                hyphenCounter++;
                continue;

            } else if ((i == stringToVerify.length() - 1) && c == 'X') {
                isbnArray[9] = 10;
            } else {
                return false;
        ...
        return (isbnArrayVerifier(isbnArray) % 11 == 0);
    }
     ...
     * + x10 * 1)
     */
    int isbnArrayVerifier(int[] isbnArray) {
        int sum = 0;

        for (int i = 0; i < 10; i++) {
            sum += isbnArray[i] * (10 - i);
        }

        return sum;
    }

    int digitCounter(String stringToVerify) {
        int digitCounter = 0;
